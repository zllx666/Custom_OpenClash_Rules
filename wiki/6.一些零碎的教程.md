...
## 2.1. 本项目方案如何切换使用 OpenClash 的 Smart 内核  

由于订阅转换后端目前还不支持 Smart 策略组，所以请按照以下两个步骤实现对 url-test 策略组的自动替换。
...
然后应用配置，OpenClash 在每次启动时即会将原有配置中的 url-test 自动测速策略组替换为 Smart 策略组。  